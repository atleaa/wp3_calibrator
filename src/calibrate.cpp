...
public:
    ...
    void callback_depth(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        d_mutex.lock();
        cv::resize(tmp_depth, src_depth, cv::Size(1920,1080), 0, 0, cv::INTER_CUBIC); // resize to 1024x768 resolution
        d_mutex.unlock();
...
public:
    ...
    void detectMarkers(cv::Mat &inputImage,cv::Mat &inputDepth, Eigen::Matrix4f & transform4x4, std::string kinect_number)
    {
        ...
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ aruco param. ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~||
        double arucoSquareDimension = 0.48; // in meters
        ...
        for (size_t i = 0; i < markerIds.size(); i++)
        {
            if (markerIds[i] == 1)
            {
                cv::aruco::drawAxis(inputImage, cameraMatrix, dist_coeffs, rotationVectors[i], translationVectors[i], 0.08);
...
int main (int argc, char** argv)
{
    ...
    std::string calibration_order_initial[] = {"2", "4", "5", "3", "6"};
